...
public class JoinVo { ... }: JoinVo라는 클래스를 선언합니다.

클래스 멤버 (Class Members):
String id, pwd, name;: 클래스에 선언된 속성(멤버 변수)으로, 각각 사용자 아이디(id), 비밀번호(pwd), 사용자 이름(name)을 나타내는 문자열 타입의 변수입니다.

생성자 (Constructor):
클래스에 명시적인 생성자가 정의되어 있지 않으므로, 기본 생성자가 사용됩니다.

접근 제어자 (Access Modifiers):
public: 클래스와 클래스 멤버가 어디서든 접근 가능하도록 하는 접근 제어자입니다. */
public class JoinVo {
  String id, pwd, name;
}
