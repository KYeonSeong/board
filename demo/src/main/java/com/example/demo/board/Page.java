...
int startNo = 1, endNo;: 현재 페이지의 시작 번호와 끝 번호를 나타내는 정수형 변수입니다.
int listSize = 7, blockSize = 5;: 한 페이지에 보여질 항목의 수와 한 블록에 보여질 페이지의 수를 나타내는 변수입니다.
int startPage = 1, endPage;: 현재 블록의 시작 페이지와 끝 페이지를 나타내는 변수입니다.
int totSize, totPage;: 전체 항목 수와 전체 페이지 수를 나타내는 변수입니다.
int nowPage = 1;: 현재 페이지 번호를 나타내는 변수입니다.
String findStr;: 검색어를 저장하는 문자열 변수입니다.

생성자 (Constructors):
public Page(){}: 기본 생성자로, 아무런 매개변수 없이 객체를 생성할 때 사용됩니다.
public Page(int totSize, int nowPage): 두 개의 매개변수를 받는 생성자로, 전체 항목 수와 현재 페이지 번호를 초기화하는 데 사용됩니다.

메서드 (Methods):
public void pageCompute(): 페이지 계산을 수행하는 메서드로, 전체 페이지 수, 현재 페이지의 시작과 끝 번호, 현재 블록의 시작과 끝 페이지를 계산합니다.

이 클래스는 전체 항목 수와 현재 페이지 번호를 받아 페이지 처리에 필요한 여러 변수를 계산하고 설정하는 데 사용됩니다. 페이지 처리 로직이 담겨있어, 페이징에 필요한 정보를 계산하는 데 활용할 수 있습니다. */
public class Page {
  int startNo = 1, endNo;
